...
### M20-017 Evidence (2026-09-02)

```text
Task: M20-017
Assignee: Codex
Started / completed: 2026-09-02 / 2026-09-02
Outcome: DeepSeek now emits one immutable StateManager-owned InferenceUsageReceipt for every provider attempt, including zero-load transport retry/cancel records. Valid provider counters retain input, cached input, output, reasoning, request, model, tokenizer-accounting version, attempt, and completion provenance; missing counters fall back explicitly to a versioned local character estimate. Added a deterministic usage-classifier.v1 that apportions raw input across declared semantic/runtime segments, counts cached semantic input without subtraction, separates reasoning and visible output, and collapses transport/runtime-repair attempts into non-experienced overhead. StateManager validates schema, exact source-closure hash, receipt/run linkage, segment conservation, attempt semantics, non-leakage flags, and idempotency before persisting receipts, breakdowns, segments, and sources.
Authority read: AGENTS.md; TODO.md; docs/invariants/19 section 3; docs/cognition/18 sections 1-7; docs/history/17 sections 1-2, 4, 7-9; inference-usage-receipt, experienced-usage-breakdown, cognitive-energy-settlement, and cognitive-runtime schemas; migration 003; M20-008 runtime ports; M20-016 energy engine.
Files changed: TODO.md; src/gf/cli.ts; src/gf/inference/base.ts; src/gf/inference/deepseekResponses.ts; src/gf/inference/usage/usageAccounting.ts; src/gf/state/stateManager.ts; src/gf/tests/deepseekResponses.test.ts; src/gf/tests/usageAccounting.test.ts.
Checks: npm test (119/119, including generated-type check); contract validation (34 schemas, 29 positive samples, 31 negative contracts, migrations 001-004); full project audit including 2758 canon entries and 10 diagrams; live synthetic DeepSeek smoke persisted provider usage (962 input, 0 cached, 2 output, 0 reasoning, completed); git diff --check.
Known residual risk: Segment weights remain an explicit call-input plan rather than provider metadata. M20-018 must bind the assembler-produced exact segment plan, CapacityEnvelope, accepted/rejected semantic result, reservation, receipt, classification, settlement, and lease release into one replayable lifecycle. The rolling request alias deepseek-v4-flash still requires model/tokenizer calibration before an accounting-version switch can settle world-side energy.
Rollback: Revert the M20-017 task commit. Migration 003 remains intact and empty usage tables are harmless; the DeepSeek adapter returns to prompt-run-only audit and M20-018 becomes blocked again.
Owner decision still needed: None.
```

### M20-018 Evidence (2026-09-02)

```text
Task: M20-018
Assignee: Codex
Started / completed: 2026-09-02 / 2026-09-02
Outcome: Added a provider-neutral CognitiveCallLifecycle that validates one consistent Wake/reservation/envelope/prompt-run boundary, commits the reservation and prompt shell before its only model await, passes CapacityEnvelope only as an engine-side argument, requires explicit provider capacity application/degradation reporting, persists and classifies the accepted receipt, proposes price-independent model/tokenizer-versioned settlement, and either settles or releases the lease. StateManager atomically commits reservation plus prompt shell, atomically commits settlement plus account CAS plus lease status, validates exact experienced-breakdown sources, conserves held energy, prevents autonomous use of protected reply reserve, and deterministically closes failed prompt runs while restoring the lease.
Authority read: AGENTS.md; TODO.md; docs/invariants/19 section 3; docs/cognition/18 sections 1-7; docs/history/17 sections 1-4 and 7-9; cognitive energy account/reservation/capacity/settlement schemas; migration 003; M20-008 runtime ports; M20-015 admission; M20-016 energy engine; M20-017 usage accounting.
Files changed: TODO.md; src/gf/cognition/energy/energyEngine.ts; src/gf/cognition/lifecycle/cognitiveCallLifecycle.ts; src/gf/state/stateManager.ts; src/gf/tests/cognitiveCallLifecycle.test.ts.
Checks: npm test (121/121, including generated-type check); contract validation and full project audit; model-await test opens a concurrent BEGIN IMMEDIATE after reservation; success test proves 50 reserved -> 25 spent + 25 released with account revision CAS; failure test proves full release and failed prompt audit; git diff --check.
...
Server handoff (2026-09-17, Codex): Owner authorized pushing accumulated fixes and a deployment plan. Added read-only live debug HTTP/ZIP endpoints, localhost-only Compose port and separate no-credential debug container, plus migration/backup/update runbook. Checks: 171/171 TypeScript tests; Python HTTP integration verifies JSON/ZIP routes, HTML escaping, 404 for filesystem paths and unchanged SQLite bytes; full project audit passes using the complete Git checkout (39 schemas, 2758 canon entries, 10 diagrams). Docker is unavailable here; image build and real server/live provider acceptance remain pending. Deployment guide: docs/operations/server-deployment.md.

Structured-output follow-up (2026-09-17, Codex): implementation DONE; 171/171 offline tests. Owner replay returned HTTP 200, completed, 37 output tokens of plain conversation, rejected as invalid_json. Policy/compiler v5 end with explicit internal JSON output tasks after dialogue. Provider errors retain redacted bounded output diagnostics; incomplete responses fail closed even if partial text parses. No unvalidated speech fallback or automatic extra inference. Live provider acceptance remains pending. See the pilot runbook.

Conversation foundation (2026-09-16, Codex): **DONE** (168/168 offline tests; live language acceptance pending), Owner authorized. Restore role-separated dialogue from perceived, source-closed evidence; preserve durable turn consumption and distinguish resume/history from new input. Provider-wire regression covers native roles and world boundaries; restart regression covers exact text and inference-time arrivals.

Temporal-context follow-up (2026-09-16, Codex): implementation **DONE**, real-model acceptance pending. Owner reported historical conversations being answered again after wakes/restarts. Preserve source occurrence time, distinguish trigger from history, deduplicate retrieval by sources, and verify restart/message replay behavior. No history deletion or fixed proactive cooldown. Evidence: 166/166 tests, including original send/receive times, reconnect ID deduplication, restart wake separation, re-observation source deduplication and event-age rendering. Contract checks pass (39 schemas). The screenshot alone does not establish the actual latest trigger; export fresh diagnostics for remaining semantic repetitions.

Conversation repair (2026-09-16, Codex): implementation **DONE**, live language acceptance pending; Owner authorized. Scope: attributed evidence, bounded atomic inbound batches, immediate self-speech perception, grounded compiler mapping and live character expression. Evidence: npm test 162/162; 39 schemas and positive/negative contract checks pass. Full project audit reaches diagram validation but cannot finish because the supplied archive omitted cognitive-wake-token-energy-v1.svg (the packaging filter excluded token-named assets); no audit bypass or fabricated diagram. See docs/operations/feishu-life-pilot.md for changed behavior and remaining model-quality limits. Mac delivery includes a source diff and hash-checked installer with backup/build rollback; no live database rewrite or outbound calls.

Usage settlement repair (2026-09-15, Codex): completed implementation and offline acceptance. The versioned life-deepseek-units.v2 profile explicitly accepts the observed deepseek-flash response alias for deepseek-v4-flash requests, preserving the provider receipt. Unknown models and mismatched tokenizers remain rejected. All 154 tests pass, including full provider/lifecycle/compilation/outbox regression for canonical and alias names plus unknown-model rejection. Contract/project audits pass; live Feishu reply acceptance still pending.

Provider compatibility repair (2026-09-15, Codex): implementation and offline checks complete; live Feishu acceptance remains pending. DeepSeek wire schemas now type enum-only nodes, omit unsupported formats, and disable provider strict mode to preserve optional-field contracts. Original Ajv validation remains mandatory. Regression checks: npm test 152/152; contract and project audits pass. Synthetic live Policy request validated; no Feishu messages sent during repair.
