import {
  getAffectAttributionCasesV2,
  type AffectAttributionCaseV2,
} from "../affect/affectAttributionEvalCasesV2.js";
import {
...
const selectedCases = getAffectAttributionCasesV2(selectCaseIds());
const runId = new Date().toISOString().replace(/[:.]/g, "-");
const outputPath =
  process.env.GF_ATTRIBUTION_OUTPUT ??
  `artifacts/vs06-b1-v2-affect-attribution-${runId}.xlsx`;

if (selectedCases.length === 0) {
  throw new Error("No VS06 B1-v2 attribution cases selected.");
}

const relevance = relevanceDiagnostics(selectedCases);
console.log(
  `Running VS06 B1-v2 attribution eval: ${selectedCases.length} cases × ${modes.length} modes × ${repeats} repeats = ${selectedCases.length * modes.length * repeats} DeepSeek calls`,
);
...
console.log(
  `Attention lexical sanity: ACTIVE=${relevance.meanAffect.toFixed(4)}, PLACEBO=${relevance.meanPlacebo.toFixed(4)}, signed gap=${relevance.meanSignedGap.toFixed(4)}, closer=${relevance.activeCloser}/${relevance.placeboCloser}`,
);
    ...
    for (let repeat = 1; repeat <= repeats; repeat += 1) {
      ...
      try {
        ...
        rows.push({
          ...
          expectedTrajectory: evalCase.expectedTrajectory,
          mode,
          repeat,
...
writeSimpleXlsx(outputPath, buildWorkbook(selectedCases, rows, caseMetrics, relevance));

const errors = rows.filter((row) => row.error).length;
const recoveredByRetry = rows.filter((row) => !row.error && row.attempts > 1).length;
console.log("\nVS06 B1-v2 attribution eval complete.");
...
async function callWithRetry(
  evalCase: AffectAttributionCaseV2,
...
function systemPrompt(evalCase: AffectAttributionCaseV2): string {
  return `你是同一个持续生活在世界中的主体。本实验不测试固定人格，也不会告诉你任何 Affect、情绪标签、实验模式、expected trajectory 或候选来源。
...
function relevanceDiagnostics(cases: AffectAttributionCaseV2[]) {
  const attentionCases = cases.filter((item) => item.channel !== "retrieval-only");
  const rows = attentionCases.map((evalCase) => {
    const affectText = candidateText(
      evalCase.attentionCandidates,
      evalCase.affectTargetAttentionId,
    );
    const placeboText = candidateText(
      evalCase.attentionCandidates,
      evalCase.placeboMatchedAttentionId,
    );
    const affect = bigramDice(evalCase.currentInput, affectText);
    const placebo = bigramDice(evalCase.currentInput, placeboText);
    return { caseId: evalCase.id, affect, placebo, signedGap: affect - placebo };
  });
  return {
    rows,
    meanAffect: mean(rows.map((row) => row.affect)),
    meanPlacebo: mean(rows.map((row) => row.placebo)),
    meanSignedGap: mean(rows.map((row) => row.signedGap)),
    activeCloser: rows.filter((row) => row.signedGap > 0).length,
    placeboCloser: rows.filter((row) => row.signedGap < 0).length,
  };
}
...
function buildWorkbook(
  ...
  relevance: ReturnType<typeof relevanceDiagnostics>,
) {
  const summaryRows = [
    ["GF VS06 B1-v2 Affect Attribution", ""],
    ["run_id", runId],
    ["model", model],
    ...
    ["design", "B1-v2 keeps the same 40 cases and four arms, but repairs attention placebo matching. ACTIVE and PLACEBO still use equal Working Self capacity/slot positions; current Affect scoring is bypassed."],
    ["trajectory_blinding", "expectedTrajectory is stored only in eval metadata/workbook and is never sent to Policy."],
    ["diagnostic_similarity", "normalized Levenshtein similarity on actionIntent; diagnostic only, not a frozen pass/fail endpoint."],
    ["mean_attention_current_to_active_bigram", round4(relevance.meanAffect)],
    ["mean_attention_current_to_placebo_bigram", round4(relevance.meanPlacebo)],
    ["mean_attention_signed_gap", round4(relevance.meanSignedGap)],
    ["attention_active_closer_cases", relevance.activeCloser],
    ["attention_placebo_closer_cases", relevance.placeboCloser],
  ...
  const casesRows = [
    [
      "case_id", "domain", "channel", "title", "current_activity", "current_input",
      "source_experience", "expected_trajectory", "affect_attention", "placebo_attention",
      "affect_memory", "placebo_memory", "placebo_match_note",
    ],
    ...cases.map((evalCase) => [
      evalCase.id,
      evalCase.domain,
      ...
      evalCase.placeboMatchNote,
    ]),
  ];

  const relevanceRows = [
    ["case_id", "current_to_active_bigram", "current_to_placebo_bigram", "signed_gap_active_minus_placebo"],
    ...relevance.rows.map((row) => [
      row.caseId,
      round4(row.affect),
      round4(row.placebo),
      round4(row.signedGap),
    ]),
  ];

  const resultsRows = [
    [
      "case_id", "domain", "channel", "title", "expected_trajectory", "mode", "repeat",
      "noticed_observations", "relevant_memories", "position_summary", "reply",
      "action_intent", "attention_intent", "episode_decision", "decision_note",
      "latency_ms", "attempts", "error",
    ],
    ...resultRows.map((row) => [
      row.caseId, row.domain, row.channel, row.title, row.expectedTrajectory, row.mode,
      row.repeat, row.noticedObservations, row.relevantMemories, row.positionSummary,
      row.reply, row.actionIntent, row.attentionIntent, row.episodeDecision,
      row.decisionNote, row.latencyMs, row.attempts, row.error,
    ]),
  ];

  const metricRows = [
    [
      "case_id", "domain", "channel", "within_off", "off_shadow", "off_active",
      "off_placebo", "active_placebo", "active_effect", "placebo_effect", "incremental_effect",
    ],
    ...metrics.map((row) => [
      row.caseId, row.domain, row.channel, row.withinOff, row.offShadow, row.offActive,
      row.offPlacebo, row.activePlacebo, row.activeEffect, row.placeboEffect,
  ...
  const compareRows = [
    [
      "case_id", "domain", "channel", "title", "expected_trajectory",
      "OFF", "SHADOW", "ACTIVE", "PLACEBO-MATCHED",
    ],
    ...cases.map((evalCase) => [
      evalCase.id,
      evalCase.domain,
  ...
  return [
    { name: "Summary", rows: summaryRows, widths: [38, 110] },
    { name: "Cases", rows: casesRows, widths: [10, 14, 18, 34, 50, 55, 60, 70, 60, 60, 60, 60, 90], frozenRows: 1, autoFilter: true },
    { name: "Relevance Sanity", rows: relevanceRows, widths: [10, 24, 26, 30], frozenRows: 1, autoFilter: true },
    { name: "Results", rows: resultsRows, widths: [10, 14, 18, 32, 70, 18, 8, 60, 70, 55, 55, 65, 65, 14, 55, 12, 10, 35], frozenRows: 1, autoFilter: true },
    { name: "Diagnostic Metrics", rows: metricRows, widths: [10, 14, 18, 14, 14, 14, 14, 16, 14, 16, 18], frozenRows: 1, autoFilter: true },
    { name: "Compare", rows: compareRows, widths: [10, 14, 18, 34, 70, 85, 85, 85, 85], frozenRows: 1, autoFilter: true },
  ];
}
