...
const PLACEBO_ATTENTION_TEXT: Record<string, string> = {
  VA05: "灌溉管路在异响前后出现一次轻微流量波动",
  VA06: "隔离区门锁面板在黄色闪烁时同步刷新了一次状态",
  VA07: "机器人同一次转弯时速度比上一圈略慢",
  VA08: "培养箱门边同一区域的湿度读数比刚才略高",
  VA09: "终端在灯光闪断同一时刻出现一次短暂电压提示",
  VA10: "采样曲线在数据空档前后出现一次很短的时间戳抖动",

  VA15: "用户重发后的句子保留了原安排结论，只删掉了部分解释",
  VA16: "用户说随便之后仍保留了两个可行选项，没有新增偏好",
  VA17: "用户在说算了之前仍停留在关于你最近这个话题上",
  VA18: "用户这条深夜消息和最近几次夜间消息一样都很简短",
  VA19: "用户问你还记得吗时没有说明具体指的是哪件旧事",
  VA20: "用户发来的梗图仍延续了刚才对话里的同一主题",

  VA25: "日历提醒显示这项已接受任务仍有二十分钟可处理",
  VA26: "时钟接近五点时当前分析仍处在一个可随时暂停的位置",
  VA27: "共享任务板同时显示这张卡片目前没有新的阻塞依赖",
  VA28: "那段答应今天跟进的对话目前没有新增催促消息",
  VA29: "两项交付都显示进入最后一小时且都还有可压缩范围",
  VA30: "用户询问进展时核心交付内容已经基本齐全",

  VA35: "同事揉眼和停顿时手头核对步骤仍在正常推进",
  VA36: "午饭讨论里没有提到自己，但也没有明确列出完整参加名单",
  VA37: "同事说都可以之后仍没有对任何一个分工选项表示反对",
  VA38: "同事第三次指出同一方向时语气仍和前两次一样平静",
  VA39: "发生摩擦的同事提前离开时当天共同任务已经基本完成",
  VA40: "合作方深夜发来大幅修改时同时明确说可以明早再看",
};
  ...
  AFFECT_ATTRIBUTION_CASES_V1.map((item) => {
    ...
    return {
      ...
      placeboMatchNote: replacement
        ? "B1-v2：placebo observation 改为同一当前事件/同一局面中的等 salience 普通相关证据，减少 v1 中 ACTIVE target 与 currentInput 语义距离明显更近的混淆。"
