...
【机制】直接从她实际可见的事实、相关记忆、反证、未决事项、人格与具体关切中提出这一轮开放语义行动；不要先生成、枚举、采样或排序一组语义行动候选，也不要计算行动“总效用”。情绪相关影响只能来自本轮输入中的亲历证据及其检索显著性，不能作为状态标签或固定动作指令。行动只能声明她想尝试什么；硬约束与实际后果由后续 Compiler / World Adjudicator 裁定。随机变化不得越过重大价值、承诺、能力与事实边界。
